...
+function ($, undefined) {
    ...
    let flag = false;
    if(flag){
        URL = 'http://api.xuemao.com'
    }else{
        URL = 'http://api.bch.xuemao.com'
    }
    $.extend($.ucse, {
        CHINACITYCLASS: URL + '/public/web/' + urlType,//列表
        CHINACITYCLASS: URL + '/xuemao/public/china_city_class.json' + urlType,//列表
        LOGIN:URL + '/xuemao/public/china_city_class.json' + urlType,//登录
...
;(function(win,doc){
    ...
    if(clientWidth > 1200){
        var rem = 40/1960*doc.documentElement.clientWidth;
    }else {
        var rem = 40/1960*1200;
    }
